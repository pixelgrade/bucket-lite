...
if($wpGrade_importerError !== false) {
	...
	if(!is_file($import_filepath.'.php')) {
		...
		'The PHP file containing the theme options data could not be found or could not be read in <pre>'.wpgrade::themefilepath('wpgrade-content/inc/import/demo-data') .'</pre><br/> You might want to try to set the file permission to 777.<br/>If this doesn\'t work please use the Theme Options Import/Export and import the .txt file provided in the archive you\'ve received on purchase manually.');
	} else {
		ob_start();
