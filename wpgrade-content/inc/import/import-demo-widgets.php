...
if($wpGrade_importerError !== false) {
	...
	if(!is_file($import_filepath.'.php')) {
		...
		'The PHP file containing the widgets data could not be found or could not be read in <pre>'.wpgrade::themefilepath('wpgrade-content/inc/import/demo-data') .'</pre><br/> You might want to try to set the file permission to 777.');
	} else {
		ob_start();
