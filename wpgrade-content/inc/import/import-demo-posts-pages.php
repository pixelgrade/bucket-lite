...
$import_filepath = WPGRADE_LIB_PATH ."inc/import/demo-data/demo_data";
...
if ( !class_exists( 'WPGrade_WP_Import' ) ) {
	$class_wp_import = WPGRADE_LIB_PATH . 'inc/import/wordpress-importer/wordpress-importer.php';
...
if($wpGrade_importerError !== false) {
	...
	if(!is_file($import_filepath.'.xml')) {
		...
		'The XML file containing the demo data could not be found or could not be read in <pre>'.WPGRADE_LIB_PATH .'inc/import/demo-data/</pre><br/> You might want to try to set the file permission to 777.<br/>If this doesn\'t work please use the <a href="'.admin_url( 'import.php' ).'">WordPress default import</a> and import the .XML file provided in the archive you\'ve received on purchase manually.');
	} else {
		ob_start();
