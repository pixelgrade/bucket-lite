...
if(!class_exists('Redux_Options') ){
    ...
    if(!defined('Redux_OPTIONS_URL')) {
		if ($fslashed_abs != '/') {
			...
			define('Redux_OPTIONS_URL', site_url($fslashed_dir));
		}
    }
    ...
    class Redux_Options {
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {    
            ...
            $defaults['google_api_key'] = 'AIzaSyB7Yj842mK5ogSiDa3eRrZUIPTzgiGopls'; // Must be defined for use with Google Webfonts field type
            $defaults['last_tab'] = '0';
            $defaults['menu_icon'] = Redux_OPTIONS_URL . '/img/menu_icon.png';
            $defaults['menu_title'] = __('Options', wpgrade::textdomain());
            $defaults['page_icon'] = 'icon-themes';
            $defaults['page_title'] = __('Options', wpgrade::textdomain());
            ...
            $defaults['footer_credit'] = sprintf(__('<span id="footer-thankyou">Options panel created using <a href="%s" target="_blank">Redux Framework</a> v%s</span>', wpgrade::textdomain()), $this->framework_url, $this->framework_version);
            $defaults['help_tabs'] = array();
            $defaults['help_sidebar'] = __('', wpgrade::textdomain());
            ...
            if($this->args['page_type'] == 'submenu') {
                ...
                if(true === $this->args['allow_sub_menu']) {
                    ...
                    if(true === $this->args['show_import_export']) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __('Import / Export', wpgrade::textdomain()),
                            __('Import / Export', wpgrade::textdomain()),
                            $this->args['page_cap'], 
                    ...
                    if(true === $this->args['dev_mode']) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __('Dev Mode Info', wpgrade::textdomain()),
                            __('Dev Mode Info', wpgrade::textdomain()),
                            $this->args['page_cap'], 
        ...
        */
        function _enqueue() {
            ...
            wp_localize_script('redux-opts-js', 'redux_opts', array('reset_confirm' => __('Are you sure? You will lose all custom values.', wpgrade::textdomain()), 'opt_name' => $this->args['opt_name']));
            ...
            foreach($this->sections as $k => $section) {
                if(isset($section['fields'])) {
                    foreach($section['fields'] as $fieldk => $field) {
                        ...
                        if(isset($field['type']) && ( $field['type']=='checkbox' || $field['type']=='checkbox_hide_below' || $field['type']=='checkbox_hide_all' || $field['type']=='checkbox_show_below' )){
        ...
        */
        function _options_page_html() {
            ...
            echo '<div id="redux-opts-header"><div class="pixelgrade_logo"></div>';
            submit_button('Save Changes', 'primary', '', false);
            // echo ' &nbsp; ';
            // submit_button('Reset to Defaults', 'secondary', $this->args['opt_name'] . '[defaults]', false);
            ...
            if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('redux-opts-saved') == '1') {
                if(isset($this->options['imported']) && $this->options['imported'] == 1) {
                    echo '<div id="redux-opts-imported">' . apply_filters('redux-opts-imported-text-' . $this->args['opt_name'], __('<strong>Settings Imported!</strong>', wpgrade::textdomain())) . '</div>';
                } else {
            ...
            echo '<div id="redux-opts-save-warn">' . apply_filters('redux-opts-changed-text-' . $this->args['opt_name'], __('<strong>Settings have changed, you should save them!</strong>', wpgrade::textdomain())) . '</div>';
            echo '<div id="redux-opts-field-errors">' . __('<strong><span></span> error(s) were found!</strong>', wpgrade::textdomain()) . '</div>';
            echo '<div id="redux-opts-field-warnings">' . __('<strong><span></span> warning(s) were found!</strong>', wpgrade::textdomain()).'</div>';
            ...
            foreach($this->sections as $k => $section) {
                if(isset($section['divider']) && $section['divider'] == true) {
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-opts-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span>' . __('Import / Export', wpgrade::textdomain()) . '</span></a>';
                echo '</li>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="redux-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span>' . __('Dev Mode Info', wpgrade::textdomain()) . '</span></a>';
                echo '</li>';
            }
            ...
            if(true === $this->args['show_import_export']) {
                ...
                echo '<h3>' . __('Import / Export Options', wpgrade::textdomain()) . '</h3>';
                echo '<h4>' . __('Import Options', wpgrade::textdomain()) . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-opts-import-code-button" class="button-secondary">' . __('Import from file', wpgrade::textdomain()) . '</a> <a href="javascript:void(0);" id="redux-opts-import-link-button" class="button-secondary">' . __('Import from URL', wpgrade::textdomain()) . '</a></p>';

                echo '<div id="redux-opts-import-code-wrapper">';

                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters('redux-opts-import-file-description', __('Input your backup file below and hit Import to restore your sites options from a backup.', wpgrade::textdomain())) . '</p>';
                echo '</div>';
                ...
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters('redux-opts-import-link-description', __('Input the URL to another sites options set and hit Import to load the options from that site.', wpgrade::textdomain())) . '</p>';
                echo '</div>';
                ...
                echo '<p id="redux-opts-import-action"><input type="submit" id="redux-opts-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __('Import', wpgrade::textdomain()) . '"> <span>' . apply_filters('redux-opts-import-warning', __('WARNING! This will overwrite any existing options, please proceed with caution!', wpgrade::textdomain())) . '</span></p>';
                echo '<div id="import_divide"></div>';

                echo '<h4>' . __('Export Options', wpgrade::textdomain()) . '</h4>';
                echo '<div class="redux-opts-section-desc">';
                echo '<p class="description">' . apply_filters('redux-opts-backup-description', __('Here you can download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', wpgrade::textdomain())).'</p>';
                echo '</div>';

//                echo '<p><a href="javascript:void(0);" id="redux-opts-export-code-copy" class="button-secondary">' . __('Copy', wpgrade::textdomain()) . '</a> <a href="'.add_query_arg(array('feed' => 'reduxopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="redux-opts-export-code-dl" class="button-primary">Download</a> <a href="javascript:void(0);" id="redux-opts-export-link" class="button-secondary">' . __('Copy Link', wpgrade::textdomain()) . '</a></p>';
				echo '<p><a href="'.add_query_arg(array('feed' => 'reduxopts-'.$this->args['opt_name'], 'action' => 'download_options', 'secret' => md5(AUTH_KEY.SECURE_AUTH_KEY)), site_url()).'" id="redux-opts-export-code-dl" class="button-primary">Download</a></p>';
            ...
            if(true === $this->args['dev_mode']) {
                ...
                echo '<h3>' . __('Dev Mode Info', wpgrade::textdomain()) . '</h3>';
                echo '<div class="redux-opts-section-desc">';
            ...
            submit_button(__('Reset to Defaults', wpgrade::textdomain()), 'secondary', $this->args['opt_name'] . '[defaults]', false);
            echo ' &nbsp; ';
