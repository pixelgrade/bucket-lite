    ...
    function render() {
        ...
        echo '<ul class="text_sortable ' . $class . '">';
	        ...
	        if ( !empty($this->field['checkboxes']) ) {
				echo '<li class="head">';
		            echo '<span class="spacer" style="display: inline-block; width: 402px;">&nbsp;</span>';
            ...
            if (isset($this->value) && is_array($this->value)) {
					...
					if (!empty($field)) {
						...
						if ( isset($this->field['checkboxes'] ) && !empty($this->field['checkboxes']) ){
							foreach ( $this->field['checkboxes'] as $key => $name ) {
								...
								echo '<input type="checkbox" name="' . $this->args['opt_name'] . '[' . $this->field['id'] . '][' . $k . '][checkboxes]['.$key.']" '. $checked .' class="social-input-checkbox" />';
							}
						}
				...
				if (!empty($options)) {
					foreach ($options as $k => $nicename) {
						...
						foreach ( $this->field['checkboxes'] as $key => $name ) {
							echo '<input type="checkbox" name="' . $this->args['opt_name'] . '[' . $this->field['id'] . '][' . $k . '][checkboxes]['.$key.']" class="social-input-checkbox" />';
						}
