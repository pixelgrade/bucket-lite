...
<?php if ( comments_open() ) : ?><div id="reviews"><?php
	...
	if ( get_option('woocommerce_enable_review_rating') == 'yes' ) {
		...
		if ( $count > 0 ) {
			...
			echo '<h2  class="reviews-title">'.sprintf( _n('%s review for %s', '%s reviews for %s', $count, wpgrade::textdomain()), '<span itemprop="ratingCount" class="count">'.$count.'</span>', wptexturize($post->post_title) ).'</h2>';
