 ...
 * So we add a transient which will be handled by the `bucket_admin_redirect_to_pixcare_install_once` action
 */
function bucket_force_redirect_to_pixcare_install_once() {
	...
	if ( empty( $plugin_version ) ) {
		set_transient( '_bucket_activation_redirect', 1 );
	}
}
add_action( 'after_switch_theme', 'bucket_force_redirect_to_pixcare_install_once' );

/**
 *  * Redirect the admin to the pixcare install page once only if the plugin is missing
 */
function bucket_admin_redirect_to_pixcare_install_once() {
	if ( ! get_transient( '_bucket_activation_redirect' ) ) {
		return;
	}
	delete_transient( '_bucket_activation_redirect' );
...
add_action( 'admin_init', 'bucket_admin_redirect_to_pixcare_install_once' );

function bucket_add_pixcare_install_page() {
	add_submenu_page( null, '', '', 'manage_options', 'pixelgrade_care-install', null );
}
add_action( 'admin_menu', 'bucket_add_pixcare_install_page' );

function bucket_pixcare_install_page() {
	...
	<head>
		...
		<title><?php _e( 'Pixelgrade Care &rsaquo; Installer', 'bucket' ); ?></title>
			...
			<div class="stepper">
				<div class="stepper__content">
					...
					<div class="section section--informative">
						...
						<h2 class="section__title u-text-center"><?php esc_html_e( 'One more lap to go..', 'bucket' ); ?></h2>
						<div class="section__content">
							<?php
							echo sprintf(
								'%s <strong>%s</strong> %s',
								esc_html__( 'Start your digital journey with our', 'bucket' ),
								esc_html__( 'Pixelgrade Care®', 'bucket' ),
								esc_html__( 'on board. It\'s the core of your experience with our WordPress theme, and we bet you are going to love it from day one. You’re in great hands, no doubt about that!', 'bucket' )
							); ?>
						</div>
						<div class="plugins">
							<div class="plugin  box  box--neutral  box--plugin-missing  box--plugin-invalidated">
								<div class="box__body">
									<h5 class="box__title"><?php esc_html_e( 'Pixelgrade Care®', 'bucket' ) ?></h5>
									<p class="box__text"><?php esc_html_e( 'Installing ...', 'bucket' ) ?></p>
								</div>
							</div>
						</div>
					</div>
					...
					<a class="btn  btn--text  btn--return-to-dashboard"
					   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'bucket' ) ?></a>
				</div>
			</div>
		</div>
	</div>
        ...
        (function ($) {
            $(document).ready(function () {
                ...
                wp.ajax.send({type: 'GET'}).always(function (res) {
                    var success = res.indexOf('<p><?php esc_html_e( 'Plugin installed successfully.', 'bucket' ); ?></p>');
                    var already_exists = res.indexOf('<p><?php esc_html_e( 'Plugin destination folder already exists.', 'bucket' ); ?></p>');
                    ...
                    if (success !== -1) {
                        ...
                        $status.text('<?php esc_html_e( 'Bummer! Something went wrong. I’m sorry but you will need to install the plugin manually. Redirecting you to the appropriate page…', 'bucket' ); ?>');
...
add_action( 'admin_init', 'bucket_pixcare_install_page' );
